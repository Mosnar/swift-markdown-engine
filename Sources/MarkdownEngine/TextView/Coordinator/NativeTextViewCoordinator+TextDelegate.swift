...
extension NativeTextViewCoordinator {
    ...
    public func textView(_ textView: NSTextView, clickedOnLink link: Any, at charIndex: Int) -> Bool {
        ...
        // AppKit didn't drop the dispatch. Set before the automatic-link branch
        // below so every path through this delegate reports it.
        (textView as? NativeTextView)?.linkClickDidFire = true
        // Automatic links are bare text with no '[[…]]' / '[text](url)' markers,
        // so the edit zone below has nothing to reveal for them — resolve them
        // first rather than letting it consume the click.
